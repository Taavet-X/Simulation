...
#TABLA X2 ###############################
frmX2TableOptions = tk.Frame(master = tab1, relief=tk.GROOVE, borderwidth=1)
btnRunX2 = tk.Button(master = frmX2TableOptions, text = "Ejecutar Prueba")
btnRunX2.grid(row = 0, column = 0)


frmX2TableOptions.grid(row = 0, column=0)

frmX2 = tk.Frame(master = tab1)
frmX2.grid( row = 1, column = 0)

label1 = tk.Label(master = frmX2, width= 10, text = "Rango", relief=tk.RAISED, borderwidth=1)
label1.grid(row = 0, column = 0)

label2 = tk.Label(master = frmX2, width= 10, text = "FO", relief=tk.RAISED, borderwidth=1)
label2.grid(row = 0, column = 1)

label3 = tk.Label(master = frmX2, width= 10, text = "FE", relief=tk.RAISED, borderwidth=1)
label3.grid(row = 0, column = 2)

label4 = tk.Label(master = frmX2, width= 10, text = "X^2i", relief=tk.RAISED, borderwidth=1)
...
for i in range(10):
    ...
    ri = str(i/10)
    rs = str((i+1)/10)
    lblRange = tk.Label(master = frmX2, width= 10, text = ri + " - " + rs, relief=tk.GROOVE, borderwidth=1)
    lblRange.grid(row = row, column = 0)

    lblFO = tk.Label(master = frmX2, width= 10, text = "", relief=tk.GROOVE, borderwidth=1)
    lblFOs.append(lblFO)
    lblFO.grid(row = row, column = 1)

    lblFE = tk.Label(master = frmX2, width= 10, text = "", relief=tk.GROOVE, borderwidth=1)
    lblFEs.append(lblFE)
    lblFE.grid(row = row, column = 2)

    lblX2 = tk.Label(master = frmX2, width= 10, text = "", relief=tk.GROOVE, borderwidth=1)
    lblX2s.append(lblX2)
    lblX2.grid(row = row, column = 3)

label1 = tk.Label(master = frmX2, width= 10, text = "x^2 calc", relief=tk.RAISED, borderwidth=1)
label1.grid(row = 11, column = 2)

lblX2Total = tk.Label(master = frmX2, width= 10, text = "", relief=tk.GROOVE, borderwidth=1)
lblX2Total.grid(row = 11, column = 3)


def setFOs(fos):
    for i in range(len(fos)):
        lblFOs[i].config(text = str(fos[i]))

def setFEs(fe):
    for i in range(len(lblFEs)):
        lblFEs[i].config(text = str(fe))

def setX2s(x2s):
    for i in range(len(x2s)):
        lblX2s[i].config(text = str(x2s[i]))

def setX2(x2):
    lblX2Total.config(text = x2)
...
def generateNumbers(event):
    ...
    if gen == "Lineal Congruente":
        try:
            ...
            numbers = GeneradorLinealCongruente.execute(x0, a, c, m, limit)
        ...
        try:
            ...
            m = int(entm.get())
            limit = int(entLimit.get())
            numbers = GeneradorEstandarMinimo.execute(x0, a, m, limit)
            txtGeneratedNumbers.delete('1.0', tk.END)
            for number in numbers:
                txtGeneratedNumbers.insert(tk.END, str(number)+"\n")
        ...
        try:
            limit = int(entLimit.get())
            numbers = RandomsPython.execute(limit)
            txtGeneratedNumbers.delete('1.0', tk.END)
            for number in numbers:
                txtGeneratedNumbers.insert(tk.END, str(number)+"\n")
        except:
